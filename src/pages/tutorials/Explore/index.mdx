...
<AnchorLinks>
  ...
  <AnchorLink>2-2: Explore kubernetes</AnchorLink>
  <AnchorLink>2-3: Exploring the Quote of the Day Application</AnchorLink>
...
Login as apmdemo-2@yahoo.com and the password provided by your instructor.
...
Explore the Host Agents on IBM i, AIX, and Linux on Power by selecting the Node and then selecting the **Open Dashboard** button.  The **IBM i** server is in a zone named **IBM i** as seen below.  The **Linux on Power** LPAR is located in the **pLinux** zone.
  ![](images/IBMi.png)
...
On the Logical Partition screens, notice that there are multiple tabs at the of the screen for **Summary**, **Network**, and **Storage**.  You'll see that key LPAR information such as the entitled processor information and whether the LPAR is capped or uncapped is captured.
...
In the upper right corner, add your name into the search widget.  You'll notice that it will filter the list to the pods related to the Quote of the Day application.
  ![](images/pods.png)
...
First, we need to filter the queries to just the database calls.  For this exercise, we'll select all database calls, but we could easily filter further to specific types of databases or specific applications.  On the left side of the screen, select **Type** and then select the **Database** checkbox.  This will filter the calls to just the **Database** calls.
  ![](images/database.png) 
...
In this exercise you have explored some of the key capabilities within Instana.  You explored PowerVM monitoring. You explored kubernetes monitoring. You explored the application monitoring details.  And, you explored how you can use unbounded analytics to glean insights.
