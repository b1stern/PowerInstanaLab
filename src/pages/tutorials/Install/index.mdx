...
<AnchorLinks>
  ...
  <AnchorLink>1.5: Install the Quote of the Day Application</AnchorLink>
...
```sh
tar -xf ./instana-agent-aix-ppc-64bit.tar.gz
```
...
Finally, scroll down to the section titled **"MariaDB"**.  Since this server is running MariaDB, let's configure that sensor.  Uncomment the section and specify the MariaDB credentials as follows. Replace \<mariadb username\> and \<mariadb passwd\> with the username and password for your environment.  The MariaDB username and password are provided in the Box Note containing the credentials for the environment. 
...
Using a terminal window, use the **oc** command to login to the cluster.  If you aren't sure how to login, you can copy the command by going to the OpenShift browser interface and select the login name in the upper right corner and then select **copy login command**
  ![](images/ocpLogin.png)
...
## 1-5: Summary
***
In this portion of the lab, you are going to install the Quote of the Day application.  During the installation, Node.js monitoring instrumentation will be enabled.  Note that for many technologies like java, monitoring instrumentation is automatic. But some technologies require some additional steps.  

In the **Instana & Turbonomic Docs** Box folder, you'll a yaml file named qotd.yaml.  Download this file and copy it to a system that has access to the **oc** command with access to the PowerVM OCP Cluster.

In the examples below, the qotd.yaml file will be placed in the /tmp directory.

Using a terminal window, use the **oc** command to login to the cluster.  If you aren't sure how to login, you can copy the command by going to the OpenShift browser interface and select the login name in the upper right corner and then select **copy login command**
  ![](images/ocpLogin.png)

First, create the required **projects(namespaces)**.   The application requires both **qotd** and **qotd-load** projects.  Issue the following commands:
```sh
oc create namespace qotd-app
oc create namespace qotd-app-load
```

Next, run the following command to install the application. If the qotd.yaml file is in a different directory, replace /tmp with the proper path.
```sh
oc apply -f /tmp/qotd.yaml
```


## 1-6: Summary

In this portion of the lab, you learned out to install the Instana Agent to monitor **AIX** and **kubernetes/OpenShift**.  You also learned some of the configuration options to customize the Agent and setup sensors that require customization.  Finally, you installed the **Quote of the Day** application.
